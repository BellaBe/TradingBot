...
class Strategy:
    ...
    def __init__(self,
                 client: typing.Union["BitmexClient", "BinanceFuturesClient"],
                 contract: Contract,
                 exchange: str,
                 timeframe: str,
                 balance_pct: float,
    ...
    def _add_log(self, msg: str):
        logger.info(f"{msg}")
        self.logs.append({"tag": msg, "displayed": False})

    def parse_trades(self, price: float, size: float, timestamp: int) -> str:
        timestamp_diff = int(time.time() * 1000) - timestamp
        if timestamp_diff >= 2000:
            logger.warning(f"{self.exchange} {self.contract.symbol}: {timestamp_diff} milliseconds of difference between the current time and the trade time")
        ...
        # Missing candles
            ...
            logger.info(f"{self.exchange} :: {missing_candles} missing candles for {self.contract.symbol} {self.timeframe} ({timestamp} {last_candle.timestamp})")
